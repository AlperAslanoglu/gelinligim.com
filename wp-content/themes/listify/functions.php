...
function listify_setup() {
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'listify_custom_background_args',
			array(
...
function listify_scripts() {
	...
	wp_enqueue_style( 'listify', listify_get_asset_src( '/css/style', 'css' ), array(), listify_get_version() );
	...
	wp_enqueue_script( 'listify', listify_get_asset_src( '/js/app', 'js' ), $deps, listify_get_version(), true );
...
$integrations = apply_filters(
	...
	array(
		...
		'wordpress-seo'                    => defined( 'WPSEO_FILE' ) && WPSEO_FILE,
	)
);
