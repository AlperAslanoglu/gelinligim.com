...
class Listify_Ratings extends Listify_Integration {
	...
	public function add_rating_field() {
		...
		/* Rating Stars */
		$default_rating = apply_filters( 'listify_ratings_default_rating', 0 ); ?>
<p class="star-rating-wrapper comment-form-rating comment-form-rating--listify">
	...
	<span class="stars">
		<?php
		for ( $i = 5; $i > 0; $i-- ) {
			printf( '<a class="star' . ( $i == $default_rating ? ' active' : '' ) . '" href="#" data-rating="%d"></a>', $i );
		} ?>
	</span>
	<input type="hidden" id="comment_rating" name="comment_rating" value="<?php echo esc_attr( $default_rating ); ?>">
	<span class="listify-badge"><?php esc_html_e( 'Oops! you forgot to give a rating.', 'listify' ); ?></span>
	 ...
	 *
	 * @return int|null The comment id or nothing.
	 */
	 ...
	 * @param string $action update ratings rate action. valid value are "add", "remove", and "recalculate"
	 */
